...
export const Rules = {
	REQ: (value : string) => regex.REQ.test(value.trim()) ? "" : message.required,
	EMAIL: (value : string) => regex.EMAIL.test(value.trim()) ? "" : message.email,
	TEL: (value : string) => regex.TEL.test(value.trim()) ? "" :  message.tel,
	ADDRESS: (value : string) => regex.ADDRESS.test(value.trim()) ? "" : message.address,
	SVN: (value : string) => {
		let check = false;
